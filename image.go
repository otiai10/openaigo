...
const (
	Size256  ImageSize = "256x256"
	Size512  ImageSize = "512x512"
	Size1024 ImageSize = "1024x1024"
)
...
type ImageUploadRequestBodyBase struct {
	// image Required
...
func (body ImageEditRequestBody) ToMultipartFormData() (buf *bytes.Buffer, err error) {
...
type ImageVariationRequestBody ImageUploadRequestBodyBase

func (body ImageVariationRequestBody) ToMultipartFormData() (buf *bytes.Buffer, err error) {
	w := multipart.NewWriter(buf)
	defer w.Close()
	imgw, err := w.CreateFormFile("image", "image.png")
	if err != nil {
